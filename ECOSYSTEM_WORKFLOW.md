...
>
> **Contexto:** El documento define la arquitectura de trabajo sincronizado entre la fuente Atomic, las tres bases híbridas auditadas y un consumidor web histórico. También establece reglas de propagación verificable para preservar la consistencia del ecosistema.
...
2. **Capa backend (Go y Rust)**:
   ...
   - Los datos simulados se limitan a pruebas automatizadas, Storybook o demos
     identificadas. No se activan por ausencia de `.env`.
...
2. La solución se codifica en `-Atomic-UI` (`.ts`, `.css`).
3. Los tokens se auditan mediante `scripts/audit-tokens.ps1`.
4. El manifiesto de fuentes se actualiza con `npm run package:manifest` cuando
   el cambio es intencional.
5. Los comandos `npm run package:check` y `npm run governance:check` se ejecutan en la fuente.
6. El cambio se propaga con clasificación exacta o con una adaptación documentada.
7. El build y las pruebas de verificación se ejecutan en cada consumidor afectado.
8. `CHANGELOG.md` se actualiza en Atomic UI con el identificador del cambio.
